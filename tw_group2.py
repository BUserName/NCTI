...
if __name__ == '__main__':
    ...
    finetune_acc = {
        ...
        'resisc': {'byol': 97.06, 'deepcluster-v2': 96.79, 'infomin': 96.57, 'insdis': 96.11, 'moco-v1': 96.22, 
                    'moco-v2': 96.46, 'pcl-v1': 96.16, 'pcl-v2': 96.17, 'sela-v2': 96.70, 'simclr-v1': 0, 
                    'simclr-v2': 0, 'swav': 96.92},
        'eurosat': {'byol': 99.37, 'deepcluster-v2': 99.26, 'infomin': 99.20, 'insdis': 99.24, 'moco-v1': 99.20, 
                    'moco-v2': 99.17, 'pcl-v1': 99.17, 'pcl-v2': 99.28, 'sela-v2': 99.31, 'simclr-v1': 0, 
                    'simclr-v2': 0, 'swav': 99.35}
        }
    dset = args.dataset
    metric = args.method
    score_path = './results_metrics/group2/{}/{}_metrics.json'.format(metric, dset)
    print(score_path)
    score, time = load_score(score_path)
    tw_sfda = w_kendall_metric(score, finetune_acc, dset)
    print("Kendall  dataset:{:12s} SFDA:{:2.3f}".format(dset, tw_sfda))
    print("Time Cost:{:.1f}".format(time))