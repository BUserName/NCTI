...
def forward_data_gradnorm(data_loader, model, configs, temperature=1):
    ...
    for b, (x, y) in enumerate(data_loader):
        ...
        model.zero_grad()
        output = model(inputs)
        target = torch.ones((inputs.shape[0], num_classes)).cuda()
...
def get_onehot_encoding(label, n):
    b = np.zeros((n, label.max() + 1))
    b[np.arange(n), label] = 1
...
def power_iteration(A, num_simulations):
    # Choose an initial approximation for the eigenvector
    ...
    for it in range(num_simulations):
        ...
        # Multiply the matrix by the normalized vector
        y_k = np.dot(A, b_k)

        # Normalize the new vector
        b_k1 = y_k / np.linalg.norm(y_k)
    ...
    largest_eigenvalue = np.dot(b_k, np.dot(A, b_k)) / np.dot(b_k, b_k)

    return largest_eigenvalue, b_k


# def rayleigh_quotient(A, num_simulations):
#     # Choose an initial approximation for the eigenvector
#     x = A.sum(axis=1)
...
#         # Update the approximation for the eigenvector
#         x = y / np.linalg.norm(y)

#         # Check for convergence
#         if np.allclose(R, np.dot(x, np.dot(A, x)) / np.dot(x, x)):
#             print(it)
#             break
...
#     return largest_eigenvalue, x


# def multiple_singular_vector_angles(U):
#     num_matrices = len(U)
#     angles = np.zeros((U, U))
#     for i in range(num_matrices):
#         for j in range(i+1, num_matrices):
#             Ui, Uj = U[i][:2], U[j][:2]
#             angle = sp.distance.cdist(Ui, Uj, 'cosine').sum()
#             print(angle)
