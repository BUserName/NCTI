...
def create_caltech101_splits(root, num_train_per_class=30):
    ...
    with open('/home/zijian/Desktop/transferability/SFDA/data/caltech101/train.txt', 'w') as f:
        for i in train_idxs:
            image_id, label = image_ids[i], labels[i]
            f.write("%s %s\n" % (image_id, label))

    with open('/home/zijian/Desktop/transferability/SFDA/data/caltech101/test.txt', 'w') as f:
        for i in test_idxs:
            image_id, label = image_ids[i], labels[i]
            f.write("%s %s\n" % (image_id, label))
