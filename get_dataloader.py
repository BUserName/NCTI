...
# from datasets.resisc import RESISC
...
def get_train_valid_loader(dset,
    ...
    if dset in [Aircraft, DTD, Flowers, VOC2007, SUN397]:
        ...
        split2 = int(num_train - np.floor(0.1 * num_train)) # test portion
        print("#######################################")
        ...
        train_idx, valid_idx = indices[split:split2], indices[:split]
        train_dataset = torch.utils.data.Subset(train_dataset, train_idx)
        valid_dataset = torch.utils.data.Subset(valid_dataset, valid_idx)
        trainval_dataset = torch.utils.data.Subset(trainval_dataset, train_idx + valid_idx)
...
def get_data(dataset):
    ...
    LINEAR_DATASETS = {
    'aircraft': [Aircraft, './data/FGVCAircraft', 100, 'mean per-class accuracy'],
    'caltech101': [Caltech101, './data/caltech101', 102, 'mean per-class accuracy'],
    'cars': [Cars, './data/stanford_cars', 196, 'accuracy'],
    'cifar10': [datasets.CIFAR10, './data', 10, 'accuracy'],
    'cifar100': [datasets.CIFAR100, './data/cifar-100-python', 100, 'accuracy'],
    'dtd': [DTD, './data/dtd/dtd', 47, 'accuracy'],
    'flowers': [Flowers, './data/flowers-102', 102, 'mean per-class accuracy'],
    'food': [Food, './data', 101, 'accuracy'],
    'pets': [Pets, './data/oxford-iiit-pet', 37, 'mean per-class accuracy'],
    'sun397': [SUN397, './data/SUN397', 397, 'accuracy'],
    'voc2007': [VOC2007, './data/VOC2007', 20, 'mAP'],
    # 'resisc': [RESISC, './data/', 45, 'accuracy'],
    # 'eurosat': [EuroSAT, './data/', 10, 'accuracy'],
    # 'fer': [FER2013, './data/', 7, 'accuracy']
    }   
